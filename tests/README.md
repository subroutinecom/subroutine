...
The schema source of truth is generated by the API into `../packages/graphql-schema/schema.graphql`
via `deno task graphql:schema` (inside `api/`) which introspects the running GraphQL server.
...
1. Read the GraphQL schema from `../packages/graphql-schema/schema.graphql`
